...
"""
Code taken from 
https://github.com/zeek/zeek-netcontrol/blob/master/openflow/controller.py
I am commenting it to try to understand it so that I can write my own 
Netcontrol based Zeek / Ryu integrated controller.
"""
...
#Defines what switches the controller is compatible with.
supported_ofctl = {
...
#broker topic location for communication between ryu controller and zeek instance.
queuename = "bro/openflow"

#zeek message parser hotfix
...
# Now we define BroController.
# Extends RyuApp class
class BroController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Our endpoint.. broker's term for a "node" that can publish or subscribe to a topic
        # And peer with other nodes. A peering is a connection between two endpoints. 
        # Peered endpoints can route messages.
        self.epl = broker.Endpoint()

    # Hook that is called after startup initialisation of app is done 
    # with __init__
    def start(self):

        #Listen on localhost port 9999 for broker peerings
        self.epl.listen("127.0.0.1", 9999)
        #We are a subscriber
        self.status_subscriber = self.epl.make_status_subscriber(True)
        #subscribed to topic queuename="bro/openflow"
